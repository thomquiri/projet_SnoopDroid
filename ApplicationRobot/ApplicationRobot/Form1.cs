...
namespace ApplicationRobot
{
    public partial class Form1 : Form
    {
        ...
        private Point? premierPointSelectionne = null;
        private Point? secondPointSelectionne = null;
        ...
        public Form1()
        {
            ...
            squareImage = Image.FromFile(@"..\..\..\..\image\ScrappyIcon.png");
            ...
            echelle = new Echelle(pictureBoxMap1, this);
            pictureBoxMap1.Paint += (sender, e) => echelle.EchelleDraw(e.Graphics);
        ...
        private void buttonCancel_Click(object sender, EventArgs e)
        {
            ...
            itineraireAuto.EnablePointAdding(false);
            echelle.CancelEchelleAdding();
            echelle.EnableEchellePointAdding(false);
            modeLigneActif = false;
        ...
        {
            pictureBoxMap1.Show();
            MessageBox.Show("Sélectionnez un plan dont les dimensions correspondent à un format paysage (taille d'un écran standard)");
        ...
        private void buttonEchelle_Click(object sender, EventArgs e)
        {
            echelle = new Echelle(pictureBoxMap1, this);
            MessageBox.Show("Veuillez sélectionner deux points sur un axe similaire. Dans le cas contraire, l'échelle peut être incorrecte.");
            echelle.ResetEchellePoints();
            pictureBoxMap1.Refresh();
            echelle.EnableEchellePointAdding(true);

            // Forcer le rafraîchissement pour assurer que les anciens dessins sont effacés
            pictureBoxMap1.Invalidate(); // Marquer la zone de dessin comme invalide pour déclencher un événement Paint
            pictureBoxMap1.Update(); // Forcer l'exécution immédiate de l'événement Paint
        }
        public void UpdateLabelEchelle(float echelleCmParPixel)
        {
            float echelleArrondie = (float)Math.Round(echelleCmParPixel, 2);
            labelEchelle.Text = $"Echelle : {echelleArrondie} cm/pixel";
        }
    }
}